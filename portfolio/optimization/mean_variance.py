"""
Mean-Variance & Cost-Aware Portfolio Optimizer for Portfolio Construction OS.
"""
...
class MeanVarianceOptimizer(BasePortfolioOptimizer):
    """Mean-Variance Optimizer solving max (w^T mu - (lambda/2) * w^T Sigma w - gamma * turnover)."""

    def __init__(self, config: Optional[Dict[str, Any]] = None):
        super().__init__("MeanVarianceOptimizer", config)
        self.risk_aversion = float(self.config.get("risk_aversion", 1.0))
        self.transaction_cost_bps = float(self.config.get("transaction_cost_bps", 10.0))
        self.cov_matrix: Optional[np.ndarray] = None
        self.asset_names: List[str] = []

    def fit(self, returns: np.ndarray, asset_names: Optional[List[str]] = None) -> "MeanVarianceOptimizer":
    ...
    def optimize(
        ...
        n = len(assets)
        mu = np.array([alpha_scores.get(a, 0.0) if alpha_scores else 0.0 for a in assets], dtype=float)
        w0 = np.ones(n) / n
        w_curr = np.array([current_weights.get(a, 0.0) for a in assets]) if current_weights else np.zeros(n)
        ...
        cost_gamma = (self.transaction_cost_bps / 10000.0)

        # Negative utility to minimize
        def objective(w):
            expected_return = float(w.T @ mu)
            variance = float(w.T @ cov @ w)
            turnover_cost = cost_gamma * np.sum(np.abs(w - w_curr))
            utility = expected_return - (0.5 * self.risk_aversion * variance) - turnover_cost
            return -utility
        ...
        if not res.success:
            return OptimizerResult(
                ...
                warnings=[f"Mean-Variance solver failed: {res.message}"],
            )
