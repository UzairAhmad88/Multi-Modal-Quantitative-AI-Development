"""
Risk Parity Portfolio Optimizer for Portfolio Construction OS.
"""
...
class RiskParityOptimizer(BasePortfolioOptimizer):
    """Solves for equal risk contribution across all portfolio assets."""

    def __init__(self, config: Optional[Dict[str, Any]] = None):
        super().__init__("RiskParityOptimizer", config)
        self.cov_matrix: Optional[np.ndarray] = None
        self.asset_names: List[str] = []

    def fit(self, returns: np.ndarray, asset_names: Optional[List[str]] = None) -> "RiskParityOptimizer":
    ...
    def optimize(
        ...
        def objective(w):
            ...
            if port_var <= 1e-12:
                return 0.0
            # Marginal risk contribution: w_i * (Sigma * w)_i
            marginal_contrib = w * (cov @ w)
            target_contrib = port_var / n
            return float(np.sum((marginal_contrib - target_contrib) ** 2))
        ...
        if not res.success:
            return OptimizerResult(
                ...
                warnings=[f"Risk Parity solver failed: {res.message}"],
            )
